...
import { IDJadiBot } from './plugins/jadibot-serbot.js';
...
global.authFile = 'IDBotSession'
global.authFileJB  = 'IDJadiBot'
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['IDBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['IDBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
browser: opcion == '1' ? ['ID-Bot', 'Edge', '20.0.04'] : methodCodeQR ? ['ID-Bot', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
version: version,
generateHighQualityLinkPreview: true
};
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "IDJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
//respaldo de la sesión "IDBotSession"
const backupCreds = () => {
...
console.log(chalk.bold.cyanBright(lenguajeID['smsConexionOFF']()))
} else if (reason === DisconnectReason.connectionClosed) {
console.log(chalk.bold.magentaBright(lenguajeID['smsConexioncerrar']()))
restoreCreds();
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.connectionLost) {
console.log(chalk.bold.blueBright(lenguajeID['smsConexionperdida']()))
restoreCreds();
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.connectionReplaced) {
console.log(chalk.bold.yellowBright(lenguajeID['smsConexionreem']()))
} else if (reason === DisconnectReason.loggedOut) {
console.log(chalk.bold.redBright(lenguajeID['smsConexionOFF']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.restartRequired) {
console.log(chalk.bold.cyanBright(lenguajeID['smsConexionreinicio']()))
await global.reloadHandler(true).catch(console.error)
} else if (reason === DisconnectReason.timedOut) {
console.log(chalk.bold.yellowBright(lenguajeID['smsConexiontiem']()))
await global.reloadHandler(true).catch(console.error) //process.send('reset')
} else {
console.log(chalk.bold.redBright(lenguajeID['smsConexiondescon'](reason, connection)))
}}
}
...
const subBotDirectory = './IDJadiBot';
...
(async () => {
global.conns = [];
const mainBotAuthFile = 'IDBotSession';
try {
...
conn.welcome = lenguajeID['smsWelcome']() 
conn.bye = lenguajeID['smsBye']() 
conn.spromote = lenguajeID['smsSpromote']() 
conn.sdemote = lenguajeID['smsSdemote']() 
conn.sDesc = lenguajeID['smsSdesc']() 
conn.sSubject = lenguajeID['smsSsubject']() 
conn.sIcon = lenguajeID['smsSicon']() 
conn.sRevoke = lenguajeID['smsSrevoke']() 
...
IDJadiBot({pathIDJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot'})
}}
}}
...
let prekey = []
let directorio = readdirSync("./IDBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./IDBotSession/${files}`)
})
} 
function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./IDJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./IDJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./IDJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./IDJadiBot/${directorio}/${fileInDir}`)
}})
}})
if (SBprekey.length === 0) {
console.log(chalk.bold.green(lenguajeID.smspurgeSessionSB1()))
} else {
console.log(chalk.bold.cyanBright(lenguajeID.smspurgeSessionSB2()))
}} catch (err) {
console.log(chalk.bold.red(lenguajeID.smspurgeSessionSB3() + err))
}}
function purgeOldFiles() {
const directories = ['./IDBotSession/', './IDJadiBot/']
...
if (err) {
console.log(chalk.bold.red(`${lenguajeID.smspurgeOldFiles3()} ${file} ${lenguajeID.smspurgeOldFiles4()}` + err))
} else {
console.log(chalk.bold.green(`${lenguajeID.smspurgeOldFiles1()} ${file} ${lenguajeID.smspurgeOldFiles2()}`))
} }) }
}) }) }) }
...
await clearTmp()
console.log(chalk.bold.cyanBright(lenguajeID.smsClearTmp()))}, 1000 * 60 * 4) // 4 min 

setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeSessionSB()
await purgeSession()
console.log(chalk.bold.cyanBright(lenguajeID.smspurgeSession()))
await purgeOldFiles()
console.log(chalk.bold.cyanBright(lenguajeID.smspurgeOldFiles()))}, 1000 * 60 * 10)

_quickTest().then(() => conn.logger.info(chalk.bold(lenguajeID['smsCargando']().trim()))).catch(console.error)

let file = fileURLToPath(import.meta.url)
watchFile(file, () => {
unwatchFile(file)
console.log(chalk.bold.greenBright(lenguajeID['smsMainBot']().trim()))
